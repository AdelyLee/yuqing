...
<script>
  ...
  export default {
    ...
    created () {
      ...
      if (!localStorage.getItem('accidentKeywords')) {
        let keywordsParams = {type: 'LOCAL'}
        getKeywords(keywordsParams).then(keywords => {
          self.keywordsConfig = keywords
          localStorage.setItem('accidentKeywords', JSON.stringify(keywords))
        })
      } else {
        self.keywordsConfig = JSON.parse(localStorage.getItem('accidentKeywords'))
      }
