<template>
  <div id="monthly-report">
    <el-col :span="24">
      <div class="card-body">
        <div class="model">
          <p class="pTitle manage-title-img">月报设置</p>
          <!-- start: 月报设置模块 -->
          <el-col :span="20" :offset="2" style="text-align: left">
            <el-form :model="addForm" ref="addForm" label-width="100px" class="monthly-report-form">
              ...
              <el-form-item label="报告接收日期" class="from-item">
                <el-col :span="12">
                  <el-select :disabled="addForm.enable ? false : true" v-model="addForm.cronExpression.day"
                             placeholder="请选择(几号)">
...
<script>
  ...
  const optionData = []
  optionData.push({value: 'L', label: '倒数第一天'})
  for (let i = 28; i > 0; i--) {
    let node = {}
    node.value = i
    node.label = i
  ...
  export default {
    ...
    methods: {
      ...
      getReportSetting() {
        let self = this
        let params = {briefingType: 'MONTHLY'}
